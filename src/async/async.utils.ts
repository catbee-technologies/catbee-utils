  ...
  function debounced(...args: Parameters<T>) {
    ...
    timer = setTimeout(() => {
      try {
        ...
        getLogger().error({ err }, 'Error in debounced function');
      }
      timer = null;
    }, delay);
  }
  ...
  debounced.flush = () => {
    if (timer) {
      ...
      try {
        ...
        getLogger().error({ err }, 'Error in debounced.flush');
      }
      timer = null;
      pendingArgs = null;
    }
  ...
  return function (...args: Parameters<T>) {
    ...
    if (remaining <= 0) {
      ...
      try {
        ...
        getLogger().error({ err }, 'Error in throttled function');
      }
    } else if (trailing) {
      savedArgs = args;
      if (!timer) {
        timer = setTimeout(() => {
          ...
          if (savedArgs) {
            try {
              ...
              getLogger().error({ err }, 'Error in throttled trailing call');
            }
          }
        }, remaining);
      }
...
export function memoizeAsync<T, Args extends any[]>(
  ...
  const generateKey = (args: Args): string => {
    try {
      ...
      getLogger().warn('memoizeAsync: key generation failed, falling back to JSON.stringify');
...
export function circuitBreaker<T, Args extends any[]>(
  ...
  return async (...args: Args): Promise<T> => {
    ...
    if (state === CircuitBreakerState.OPEN && now < unlockAt) {
 ...
 */
export function optionalRequire<T = any>(name: string): T | null {
  try {
    return require(name);
  } catch {
    return null;
