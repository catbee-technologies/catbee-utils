...
import { DependencyErrors } from '@catbee/utils/server';
...
class RateLimiterCache {
  private readonly rateLimit: typeof import('express-rate-limit').rateLimit | undefined;

  private readonly cache: TTLCache<string, CachedRateLimiter>;
  ...
  constructor(maxSize = 100, ttlMs = 5 * 60 * 1000) {
    ...
    const rateLimitModule = optionalRequire('express-rate-limit') as typeof import('express-rate-limit') | null;
  ...
  get(options: {
    ...
    if (!this.rateLimit) {
      throw new Error(DependencyErrors['express-rate-limit']);
    }
...
export class DIContainer {
  private readonly instances = new Map<Constructor, any>();
  private readonly constructing = new Map<Constructor, any>();
  private readonly propertyInjections = new Map<Constructor, PropertyInjection[]>();
