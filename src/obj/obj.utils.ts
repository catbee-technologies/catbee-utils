...
/**
 * Create a deep clone of a value. Uses structuredClone when available, falls back to JSON
 * serialization as a best-effort fallback (may lose functions and special objects).
 *
 * @template T
 * @param {T} value
 * @returns {T}
...
export function deepClone<T>(value: T): T {
  try {
    ...
    if (typeof globalThis.structuredClone === 'function') {
      // use structuredClone if available (preserves many built-in types)
