...
import fs from 'fs';
import { defaultCatbeeConfig, defaultServerConfig } from '@catbee/utils/config';
 ...
 */
export const getDependencyErrorMessage = (packageName: string, x?: string) =>
  `Missing required dependency ${x ? `for ${x}` : ''}: ${packageName}. Please install it to proceed.`;
...
export class ExpressServer {
  ...
  constructor(config: Partial<ServerConfig>, hooks: ServerHooks = {}) {
    if (this.isBuiltServerConfig(config)) {
  ...
  public async ready(): Promise<boolean> {
    try {
      if (!this.healthChecks.length || defaultCatbeeConfig.server.skipHealthz) return true;
  ...
  private isBuiltServerConfig(config: Partial<ServerConfig>): boolean {
