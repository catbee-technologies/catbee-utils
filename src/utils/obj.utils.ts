...
export function setValueByPath<T extends object>(obj: T, path: string, value: any): T {
  ...
  // Dangerous keys that can modify Object.prototype
  ...
  for (let i = 0; i < parts.length - 1; i++) {
    ...
    if (forbiddenKeys.has(key)) return obj; // Prevent prototype pollution

    const nextKey = parts[i + 1];
    const isArrayIndex = !isNaN(Number(nextKey));

    if (current[key] === undefined) {
      // Create safe objects with no prototype
