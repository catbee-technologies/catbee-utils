...
describe('IdUtils', () => {
  describe('uuid', () => {
    it('returns the result of randomUUID()', () => {
      const spy = jest.spyOn(require('crypto'), 'randomUUID').mockReturnValue('deadbeef-dead-beef-dead-beefdeadbeef');
  ...
  describe('nanoid', () => {
    ...
    it('is url-safe (no +, /, =)', () => {
      const spy = jest.spyOn(require('crypto'), 'randomBytes');
      spy.mockReturnValue(Buffer.from([255, 254, 253, 252, 251, 250, 249, 248]));
  ...
  describe('randomHex', () => {
    it('returns correct hex length for default and custom byteLength', () => {
      const spy = jest.spyOn(require('crypto'), 'randomBytes');
      spy.mockReturnValue(Buffer.from(Array.from({ length: 16 }, (_, i) => i + 1)));
      expect(randomHex()).toBe(Buffer.from(Array.from({ length: 16 }, (_, i) => i + 1)).toString('hex'));
